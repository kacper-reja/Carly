...
export default function Bookings({ active }) {
  ...
  const handleSearch = async ()=>{
    if (searchFilter===''){
      ...
      const response = await getBookingsFilter(pageNum-1,10,searchFilter.toLowerCase())
      setBooking(response.data)
    }
  }
...
useEffect(()=>{ handleSearch()},[pageNum])
  // ---- sorting ---
  const sortByModel=()=>{
      ...
      <div className="table-responsive">
        <table className="table table-striped">
          <thead>
            <tr>
              <th>
                <button className="btn" onClick={sortById}>ID &#8595; </button>
              </th>
              <th>
                <button className="btn" onClick={()=>sortByFirstName()}>First name &#8693;</button>
              </th>
              <th>
                <button className="btn" onClick={()=>sortByLastName()}>Last name &#8693;</button>
              </th>
              <th>
                <button className="btn" onClick={()=>sortByDate()}>Booking date &#8693;</button>
              </th>
              <th>
                <button className="btn" onClick={()=>sortByCarName()}>Name &#8693;</button>
              </th>
              <th>
                <button className="btn" onClick={()=>sortByModel()}>Model &#8693;</button>
              </th>
              <th>Location</th>
              <th>Action</th>
            </tr>
